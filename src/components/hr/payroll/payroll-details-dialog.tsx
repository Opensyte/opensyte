...
export function PayrollDetailsDialog({ open, onOpenChange, payroll }: Props) {
  ...
  const fmtMoney = (v: string | number | null | undefined, c: string) => {
    const num =
      typeof v === "number" ? v : v != null ? Number.parseFloat(v) : 0;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Read-only view of payroll information.
          </DialogDescription>
        </DialogHeader>
        {!payroll ? (
          ...
          <ScrollArea className="max-h-[70vh] pr-2">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Row
                  label="Pay Date"
                  value={fmtDate(payroll.payDate)?.toLocaleDateString() ?? "—"}
                />
                <Row label="Currency" value={payroll.currency} />
                <Row
                  label="Period Start"
                  value={
                    fmtDate(payroll.payPeriodStart)?.toLocaleDateString() ?? "—"
                  }
                />
                <Row
                  label="Period End"
                  value={
                    fmtDate(payroll.payPeriodEnd)?.toLocaleDateString() ?? "—"
                  }
                />
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Row
                  label="Basic Salary"
                  value={fmtMoney(payroll.basicSalary, payroll.currency)}
                />
                <Row
                  label="Overtime"
                  value={fmtMoney(payroll.overtime, payroll.currency)}
                />
                <Row
                  label="Bonus"
                  value={fmtMoney(payroll.bonus, payroll.currency)}
                />
                <Row
                  label="Tax"
                  value={fmtMoney(payroll.tax, payroll.currency)}
                />
                <Row
                  label="Deductions"
                  value={fmtMoney(payroll.deductions, payroll.currency)}
                />
                <Row
                  label="Net Amount"
                  value={fmtMoney(payroll.netAmount, payroll.currency)}
                />
              </div>
